...
public class DragNDropInventory : MonoBehaviour
{
    #region Variables
    [Header("Inventory")]
    public bool showInv;
    ...
    {
        droppedItem = draggedItem.MeshType;
    ...
    // Attributes of each item in inventory
    #region ToolTip 
    ...
    #endregion
    #region ToolTipContentWindow
    void DrawToolTip(int Window)
    {
    ...
    void InventoryDrag(int windowID)
    {
        ...
        GUI.Box(new Rect(0, scr.y * 4.25f, scr.x * 6f, scr.y * 0.5f), "Gold Display");
        showToolTip = false;
        ...
        for (int y = 0; y < slotY; y++)
        {
            ...
            {
                float slotLocationX = scr.x * 0.125f + x * (scr.x * 0.75f);
                float slotLocationY = scr.y * 0.75f + y * (scr.y * 0.65f);
                float slotLocationWidth = scr.x * 0.75f;
                float slotLocationHeight = scr.y * 0.65f;
                Rect slotLocation = new Rect( slotLocationX, slotLocationY, slotLocationWidth, slotLocationHeight);
